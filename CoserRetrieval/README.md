...
- **实测边界样本占比偏高**：新增 `apps/scan_authenticity_cli.cpp`（独立小工具，只跑 CLIP 判定不做完整
  ingest，用于批量扫描目录）对 `testdata_eval` 全量扫描：`store`（83张）23张判定为插画/CG（~28%），
  `query`（81张）23张判定为插画/CG（~28%）。但按 `|score|` 排序后发现，这批"插画"里只有约10张
  （store 7张、query 3张，`|score|>0.02`，例如已知的 `rioko_query_038.jpg` score=-0.027）分数明显偏负，
  大概率是真插画；其余约36张分数集中在 -0.02~0（接近0，两个提示语相似度几乎打平），更可能是
  磨皮/美颜/特效导致误判的真人照片，而不是真插画——即"真人 vs 插画"二分类在这批评测数据上的边界样本
  比例远高于预期，score 绝对值本身可以作为置信度的粗略参考（越接近0越不可信），但阈值需要人工标注一批
  样本后才能定下来。**本轮未据此清理数据集**（避免误删真人照片污染评测基准），只记录现象，后续如果要
  实际启用"清洗数据集"这一步，需要先人工标注这36张边界样本的真实类别，再决定阈值或换更高精度的视觉编码器。
